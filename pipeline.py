...
def transform(df):
    ...
    # Drop tz-aware for SQLite compatibility
...
def load_to_sqlite(df, rej_df, db_path="predixion.db"):
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS calls (
            ...
            agent_id TEXT, customer_phone TEXT,
            start_time TEXT, end_time TEXT,
            call_outcome TEXT, language TEXT,
            disposition_code TEXT, amount_promised REAL,
            retry_flag INTEGER, call_duration_seconds INTEGER,
            call_hour INTEGER, call_date TEXT,
            is_weekend INTEGER, duration_bucket TEXT,
            is_amount_imputed INTEGER
        )""")

    cur.execute("""
        CREATE TABLE IF NOT EXISTS ingestion_log (
            run_ts TEXT, records_processed INTEGER, rejected_count INTEGER
        )""")
    ...
    cur.execute("INSERT INTO ingestion_log VALUES (?,?,?)",
                (datetime.now().isoformat(), len(df), len(rej_df)))

    con.commit()
    con.close()
    print(f"\nLoaded {len(df)} records → {db_path}")
  
  ## Main runner
  if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser()
    ap.add_argument("--input",   default="raw_calls.json")
    ap.add_argument("--db-path", default="predixion.db")
    args = ap.parse_args()
